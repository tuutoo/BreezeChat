...
export async function POST(req: Request) {
  try {
    ...
    if (additionalPrompts.length > 0) {
      const additionalPromptTexts = additionalPrompts
        .map((prompt: { prompt: string }) => prompt.prompt)
        .join("\n");
      if (systemPrompt) {
      ...
      console.log(
        ...
        additionalPrompts.map((p: { name: string }) => p.name).join(", "),
      );
    }

    console.log(
      "Final system prompt:",
      systemPrompt || "(No system prompt - free chat)",
    );
    ...
    const providerConfig = PROVIDERS.find(
      (p) => p.providerName === model.providerName,
    );
    if (!providerConfig) {
    ...
    switch (providerConfig.providerName) {
      ...
      case "groq":
        ...
        break;
      case "openrouter":
        aiProvider = createOpenRouter({ apiKey })(model.modelId);
        break;
      default:
    ...
    const isImageCapableModel =
      providerConfig.providerName === "google" &&
      (model.modelId.includes("image") ||
        model.modelId.includes("vision") ||
        model.modelId.includes("flash-image"));
    ...
    if (isImageCapableModel) {
      ...
      try {
        ...
        const mayBeImageRequest = imageKeywords.some((keyword) =>
          content.includes(keyword),
        );

        if (mayBeImageRequest) {
          console.log(
            "Trying generateText for potential image generation with model:",
            model.modelId,
          );
          ...
          if (result.steps && result.steps.length > 0) {
            ...
            console.log(
              "All parts from steps:",
              allParts.map((p) => ({ type: p.type, hasImage: !!p.image })),
            );

            // 查找图片部分
            const imageParts = allParts.filter(
              (part) =>
                part.type === "image" || (part.image && part.type !== "text"),
            );
          ...
          if (result.files && result.files.length > 0) {
            ...
            if (imageFiles.length > 0) {
              ...
              // 构造包含文本和图片的响应
              const parts: any[] = [{ type: "text", text: result.text }];

              // 添加所有生成的图片
              for (const file of imageFiles) {
                const fileData = file as any; // 暂时类型转换处理
                console.log(
                  "Processing image file with base64Data:",
                  !!fileData.base64Data,
                );
                ...
                if (fileData.base64Data) {
                  ...
                  console.error(
                    "No base64Data or url found in file:",
